...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit* invoker)
{
    ...
    {
        mEscortNPCFlags = me->GetUInt32Value(UNIT_FIELD_NPC_FLAGS);
        me->SetFlag(UNIT_FIELD_NPC_FLAGS, 0);
    }
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        TC_LOG_DEBUG("misc", "SmartAI::StartPath: Creature entry %u wanted to pause waypoint (current waypoint: %u) movement while already paused, ignoring.", me->GetEntry(), mCurrentWPID);
        return;
    }
...
void SmartAI::EndPath(bool fail)
{
    ...
    if (mEscortNPCFlags)
    {
        me->SetFlag(UNIT_FIELD_NPC_FLAGS, mEscortNPCFlags);
        mEscortNPCFlags = 0;
    }
    ...
    // End Path events should only be processed if it was SUCCESSFUL stop or stop called by SMART_ACTION_WAYPOINT_STOP
    if (fail)
        return;
